...
#define WRITE_MSG(format_begin, format_end, name, value) {                           \
    if (!kprofiler::atrace::ATrace::Get().IsATraceStarted()) {                       \
      return;                                                                        \
    }                                                                                \
    if (kprofiler::atrace::TraceProvider::Get().isMainThreadOnly()                 \
     && kprofiler::atrace::TraceProvider::Get().GetMainThreadId() != gettid()) {    \
    return;                                                                          \
    }                                                                                \
    ...
    kprofiler::atrace::ATrace::Get().LogTrace(buf, len);                \
}
